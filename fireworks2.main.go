...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	for _, p := range activeParticles {
		...
		// --- 1. Lifetime Color Transition (Blue -> Red) ---
		// rate = 0 (Start) -> B=1.0, R=0.0 (Pure Blue)
		// rate = 1 (End)   -> B=0.0, R=1.0 (Pure Red)
		r := float32(rate)           // Red increases with life
		gcol := float32(0.0)         // Keep green at zero for a clean transition
		b := float32(1.0 - rate)     // Blue decreases with life
